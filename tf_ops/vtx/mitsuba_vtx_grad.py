...
if not "MTSTF_REDUCTOR_ENABLE" in os.environ:
    mitsuba_grad_module = tf.load_op_library(dirname + '/../mitsuba_v2_vtx_grad_op.so')
else:
    mitsuba_grad_module = tf.load_op_library(dirname + '/../mitsuba_v2_reductor_grad_op.so')
...
def _inner_product_grad_cc(op, grad):
    ...
    return mitsuba_grad_module.mitsuba_grad(grad, op.inputs[0], op.inputs[1], op.inputs[2], op.inputs[3], op.inputs[4], op.inputs[5])
