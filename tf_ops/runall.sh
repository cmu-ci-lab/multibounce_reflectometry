...
for DEPTH in "${depths[@]}"
do
    $MTSTF_LOC/build/release/mitsuba/mtstensorflow $SCENE_LOC/$FILENAME.xml -p 6 -o "/tmp/mtsout.hds" -l 7554 -Dweight1=0.5 -Dweight2=0.5 -Dalpha=0.1 -Ddepth=4 -DsampleCount=64 > /dev/null & export APP_PID=$!
#MTSTF_PID=$!
    $MTSTF_LOC/build/release/mitsuba/mtstensorflow $SCENE_LOC/$FILENAME-diff.xml -p 6 -o "/tmp/mtsgradout.hds" -l 7555 -Ddiff=0 -Dweight1=0.5 -Dweight2=0.5 -Dalpha=0.1 -Ddepth=4 -DsampleCount=64 > /dev/null & export APP_PID_2=$!
    #MTSTFDIFF_PID=$!
    sleep 2
