...
mitsuba_grad_module = tf.load_op_library('./mitsuba_v2_sparse_grad_op.so')
...
def _inner_product_grad_cc(op, grad):
    ...
    return mitsuba_grad_module.mitsuba_grad(grad, op.inputs[0], op.inputs[1], op.inputs[2], op.inputs[3])
