...
filenames=(tf_sphere_envmap_test)
#sizes=(1 2 4 8)
...
#    let COUNTER=COUNTER+1 
    ...
    qsub -V -o "${FILE}.o${JOB_ID}" -e "${FILE}.e${JOB_ID}" -b y -cwd -v DEPTH="${DEPTH}",SIZE="${SIZE}",SAMPLES="${SAMPLES}",FILENAME="${FILENAME}" "./sparse_task.sh"
