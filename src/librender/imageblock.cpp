...
ImageBlock::ImageBlock(Bitmap::EPixelFormat fmt, const Vector2i &size,
        const ReconstructionFilter *filter, const Sampler *sampler, int channels, bool warn) : m_offset(0),
        m_size(size), m_filter(filter), m_sampler(sampler), m_weightsX(NULL), m_weightsY(NULL), m_warn(warn) {
