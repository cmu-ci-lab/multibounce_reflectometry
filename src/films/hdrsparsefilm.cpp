...
class HDRSparseFilm : public Film {
public:
    HDRSparseFilm(const Properties &props) : Film(props) {
        ...
        if (m_pixelFormats.size() == 1) {
            m_storage = new SparseImageBlock(Bitmap::ESpectrumAlphaWeight, m_cropSize);
        } else {
            m_storage = new SparseImageBlock(Bitmap::EMultiSpectrumAlphaWeight, m_cropSize,
                NULL, (int) (SPECTRUM_SAMPLES * m_pixelFormats.size() + 2));
        }
    }

    HDRSparseFilm(Stream *stream, InstanceManager *manager)
        : Film(stream, manager) {
    ...
    void put(const ImageBlock *block) {
        ...
        const SparseImageBlock *sp_block = reinterpret_cast<const SparseImageBlock*>(block);
    ...
    void develop(const Scene *scene, Float renderTime) {
        ...
        else if (m_fileFormat == Bitmap::EHDRStream)*/
	...
	storage_tmp.clear();
        ...
        // TODO: Write the width and height here.
        ...
        for(std::map<std::tuple<int, int, int>, Spectrum>::iterator it = storage.begin(); it != storage.end(); it++ ) {
            ...
            if(std::isinf(v[0]) || std::isinf(v[1]) || std::isinf(v[2])) 
                Log(EInfo, "%d %d %d -> %f %f %f", std::get<0>(t), std::get<1>(t), std::get<2>(t), v[0], v[1], v[2]);
            
            stream->writeInt(std::get<0>(t));
            stream->writeInt(std::get<1>(t));
            stream->writeInt(std::get<2>(t));
...
protected:
    ...
    bool m_attachLog;
    fs::path m_destFile;
    ref<SparseImageBlock> m_storage;
};

MTS_IMPLEMENT_CLASS_S(HDRSparseFilm, false, Film)
MTS_EXPORT_PLUGIN(HDRSparseFilm, "High dynamic range sparse multidimensional texture film");
MTS_NAMESPACE_END
