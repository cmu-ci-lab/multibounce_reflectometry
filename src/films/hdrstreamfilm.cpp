...
public:
    HDRFilm(const Properties &props) : Film(props) {
        ...
        if (m_fileFormat == Bitmap::EPFM) {
            ...
            Log(EError, "Only the PFM fileformat is supported for streaming.");
        }
    ...
    void develop(const Scene *scene, Float renderTime) {
        ...
        if (m_pixelFormats.size() == 1) {
            ...
            Log(EError, "Multi-channel streaming not supported as of yet.");
            return;
        }
        ...
        Log(EInfo, "Writing image to \"%s\" ..", m_destFile.string().c_str());
