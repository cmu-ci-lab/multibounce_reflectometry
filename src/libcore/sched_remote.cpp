...
void StreamBackend::run() {
    ...
    if (m_stream->getClass()->derivesFrom(MTS_CLASS(SocketStream))) {
        ...
        Log(EInfo, "Incoming from %s", sstream->getPeer().c_str());
    }
    ...
    Log(EWarn, "SPECTURM_SAMPLES: %d", SPECTRUM_SAMPLES);
    Log(EWarn, "REfdata: %d", refData[dataLength-1]);
    if (memcmp(data, refData, dataLength) != 0) {
        m_stream->writeShort(EIncompatible);
        m_stream->flush();
        Log(EWarn, "The client either the wrong version "
