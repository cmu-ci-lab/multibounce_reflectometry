...
public:
    ...
    */
	...
	std::map<int,Spectrum>  eval_diff(const BSDFSamplingRecord &bRec, EMeasure measure) const {
        ...
        auto vec = m_reflectance->eval(bRec.its).average() * bRec.its.color.average() * (INV_PI * constrainedDotGradient(bRec.wo, Vector(0, 0, 1)));
