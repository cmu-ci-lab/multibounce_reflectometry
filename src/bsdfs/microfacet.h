...
public:
    ...
    inline void scaleAlpha(Float value) {
		...
		/* 
		 * Temporary helper function to compute the derivative of the GGX distribution with respect to the  
		 * roughness alpha.
		*/
		Float G1_diff(const Vector &v, const Vector &m) const {
			if (m_type != EGGX)
				return 0.0f;
            ...
            Float nm = Frame::cosTheta(v);
			Float a = m_alphaU;

			// Plugged in from matlab.
			return -(nm*(-2 * a*pow(nm, 2) + 2 * a)) / (pow(nm + sqrt(pow(a, 2) - pow(nm, 2) * (pow(a, 2) - 1)), 2) * sqrt(pow(a, 2) - pow(nm, 2) * (pow(a, 2) - 1)));
		}

		inline Float G_diff(const Vector &wi, const Vector &wo, const Vector &m) const {
			return G1_diff(wi, m) * smithG1(wo, m) + G1_diff(wo, m) * smithG1(wi, m);
		}
		 ...
		 * normals.
		*/
		Float G1_ndiff(const Vector &v, const Vector &m) const {
			if (m_type != EGGX)
				return 0.0f;
			...
			// Plugged in from matlab.
			//return -(nm*(-2 * a*pow(nm, 2) + 2 * a)) / (pow(nm + sqrt(pow(a, 2) - pow(nm, 2) * (pow(a, 2) - 1)), 2) * sqrt(pow(a, 2) - pow(nm, 2) * (pow(a, 2) - 1)));
            return 2/(nm + sqrt(a*a - (nm*nm)*(a*a - 1))) + (2*nm*((nm*(a*a - 1))/sqrt(a*a - (nm*nm)*(a*a - 1)) - 1))/pow(nm + sqrt(a*a - (nm*nm)*(a*a - 1)), 2);

		}

		inline Vector G_ndiff(const Vector &wi, const Vector &wo, const Vector &m) const {
			return G1_ndiff(wi, m) * smithG1(wo, m) * constrainedDotGradient(wi, Vector(0, 0, 1)) + G1_ndiff(wo, m) * smithG1(wi, m) * constrainedDotGradient(wo, Vector(0, 0, 1));
		}

		Float D_diff(const Vector &m) const {
			...
			// Plugged in from matlab.
			return -(2 * a*(a2 * nm2 + nm2 - 1)) / (M_PI*pow(a2 * nm2 - nm2 + 1, 3));

		}
		
        Vector D_ndiff(const Vector &m) const {
			if (Frame::cosTheta(m) <= 0) {
				return Vector(0.0f);
			}
			...
			// Plugged in from matlab.
			//return -(2 * a*(a2 * nm2 + nm2 - 1)) / (M_PI*pow(a2 * nm2 - nm2 + 1, 3));
            return -static_cast<float>((4*a*a*nm*(a*a - 1))/(M_PI*pow((a*a - 1)*nm*nm + 1,3))) * constrainedDotGradient(m, Vector(0, 0, 1));
    ...
    /// Compute constrained gradient of the dot product between two normalized vectors.
    inline Vector constrainedDotGradient(const Vector& v, const Vector& n) const {
        return cross(n, cross(v, n));
    }
