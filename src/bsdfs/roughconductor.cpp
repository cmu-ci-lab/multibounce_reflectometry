...
public:
    RoughConductor(const Properties &props) : BSDF(props) {
        ...
        m_normalMap = new ConstantSpectrumTexture(
            props.getSpectrum("normalMap", Spectrum(1.0f)));
    ...
    Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
		...
		/*eval_diff(const BSDFSamplingRecord &bRec,
			EMeasure measure = ESolidAngle) const {
            // Empty differential set.
			return std::map<int,Spectrum>();
		};*/
        virtual int getDifferentiableParameters() {
            // Default, no differentiables.
            return 2;
        }
    ...
    */
		...
		std::map<int,Spectrum>  eval_diff(const BSDFSamplingRecord &bRec, EMeasure measure) const {
			...
			if (measure != ESolidAngle ||
				...
				//sp.push_back(Spectrum(0.0));
				//sp.push_back(Spectrum(0.0));
                diffs[0] = Spectrum(0.0f);
                diffs[1] = Spectrum(0.0f);
				return diffs;
			}
				

			// TODO: Compute differential.
			...
			//std::vector<Spectrum> diff;
			//diff.push_back(Spectrum(0.0));
            

			// We can keep F and the denominator. D and G change with alpha.
			// Compute using := ((D * Gl * Gv' + Gl' * Gv * D + Gl * Gv * D') * F) / denom
			...
			const Float D = distr.eval(H);
			if (D == 0) {
				diffs[0] = Spectrum(0.0);
                diffs[1] = Spectrum(0.0);
				return diffs;
			}
			...
			/* Compute D prime */
			const Float Dprime = distr.D_diff(H);

			/* Compute G_prime */
			const Float Gprime = distr.G_diff(bRec.wi, bRec.wo, H);

			/* Calculate the total amount of reflection */
			Float model = (Dprime * G + Gprime * D) / (4.0f * Frame::cosTheta(bRec.wi));

			diffs[0] = Spectrum(F.getLuminance() * model);

            /* Compute normal differential */

            /* Compute dG/dn */
            const Vector GprimeDn = distr.G_ndiff(bRec.wi, bRec.wo, H);

            /* Compute dD/dn */
            const Vector DprimeDn = distr.D_ndiff(H);

            /* Compute numerator differential */
            const Vector dN = (DprimeDn * G + GprimeDn * D);
            //const Vector dN = (DprimeDn * G) / (4.0f * Frame::cosTheta(bRec.wi));

            /* Compute denominator side differential */
            //const Vector dD = (D * G * distr.constrainedDotGradient(bRec.wi, Vector(0, 0, 1))) / (4.0 * pow(Frame::cosTheta(bRec.wi),2));
            const Vector dD = Vector3f(0.0); // Disable this gradient.

            /* Compute the total differential using multiplicative formula */
            const Vector dT = (dN - dD) * F.getLuminance();

            Float vec[3] = {dT.x, dT.y, dT.z}; 
            /* Place it into the second slot */
            diffs[1] = Spectrum(vec);

            /*if(!std::isfinite(diffs[1][0])) {
                Log(EWarn, "Invalid value: %f, dN:%s, dD:%s, GprimeDn: %s, DprimeDn: %s, dT:%s, H:%s, alpha:%f", diffs[1][0], dN.toString().c_str(), dD.toString().c_str(), GprimeDn.toString().c_str(), DprimeDn.toString().c_str(), dT.toString().c_str(), H.toString().c_str(), distr.getAlpha());
            }*/
