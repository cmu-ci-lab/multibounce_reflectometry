...
public:
    SmoothDiffuse(const Properties &props)
        ...
        // get file name tag. If not specified, then use "default.dat" as default file name.
        m_bsdfFilename = props.getString("bsdfFile", "default.dat");

        ifstream bsdfFile = ifstream(m_bsdfFilename);
        int rows;
        int cols;
        bsdfFile >> rows;
        bsdfFile >> cols;

        //std::vector<Spectrum> bsdfDatapoints;

        for(int i = 0; i < rows * cols; i++) {
            float f;
            bsdfFile >> f;
            m_bsdfDatapoints.push_back(f);
        }
        
        //int N;
        
        // We have some datapoints now, and a X, Y resolution.
...
private:
    ...
    std::vector<float> m_bsdfDatapoints;
};
