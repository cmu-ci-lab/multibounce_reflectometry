...
/*!\plugin{directional}{Orthographic emitter}
 * \icon{emitter_directional}
 * \order{4}
 * \parameters{
 *     \parameter{toWorld}{\Transform\Or\Animation}{
 *        Specifies an optional emitter-to-world transformation.
 *				Use the scale attribute to specify the sie of the orthographic beam.
...
class OrthographicEmitter : public Emitter {
public:
	OrthographicEmitter(const Properties &props) : Emitter(props) {
    ...
    void configure() {
        ...
        //Float surfaceArea = M_PI * m_bsphere.radius * m_bsphere.radius;
				Vector xt = trafo(Vector(1, 0, 0));
				Vector yt = trafo(Vector(0, 1, 0));
				Float surfaceArea = cross(xt, yt).length();
    ...
    Spectrum samplePosition(PositionSamplingRecord &pRec, const Point2 &sample, const Point2 *extra) const {
        ...
        //Point2 p = warp::squareToUniformDiskConcentric(sample);
				Point3 p = Point3(2 * sample.x - 1, 2 * sample.y - 1, 0);
    ...
    Spectrum sampleRay(Ray &ray,
        ...
        //Point2 p = warp::squareToUniformDiskConcentric(spatialSample);
				Point3 p = Point3(2 * spatialSample.x - 1, 2 * spatialSample.y - 1, 0);

        //Vector perpOffset = trafo(Vector(p.x, p.y, 0) * m_bsphere.radius);
        Vector d = trafo(Vector(0, 0, 1));
				
        //ray.setOrigin(m_bsphere.center - d*m_bsphere.radius + perpOffset);
				ray.setOrigin(trafo(p));
        ray.setDirection(d);
        ray.setTime(time);
        return m_power;
    }
    ...
    Spectrum sampleDirect(DirectSamplingRecord &dRec, const Point2 &sample) const {
        ...
        //Float distance = dot(dRec.ref - diskCenter, d);
				Point3 lspt = invtrafo(dRec.ref);
				if (lspt.x < -1 || lspt.y < -1 || lspt.x > 1 || lspt.y > 1 || lspt.z < 0) {
...
class OrthographicEmitterShader : public Shader {
public:
	OrthographicEmitterShader(Renderer *renderer)
...
Shader *OrthographicEmitter::createShader(Renderer *renderer) const {
    return new OrthographicEmitterShader(renderer);
}

MTS_IMPLEMENT_CLASS(OrthographicEmitterShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(OrthographicEmitter, false, Emitter)
