...
public:
    ...
    ref<WorkResult> createWorkResult() const {
        return new BDPTWorkResult(m_config, m_rfilter.get(), m_sampler.get(),
    ...
    void evaluate(BDPTWorkResult *wr,
        ...
        Float sourceDensity = m_sampler->getSampleCount(initialDiscretePos);
        wr->putSample(initialSamplePos, sampleValue, sourceDensity);
    }
...
void BDPTProcess::bindResource(const std::string &name, int id) {
    ...
    if (name == "sensor" && m_config.lightImage) {
        ...
        m_result = new BDPTWorkResult(m_config, NULL, NULL, m_film->getCropSize());
        m_result->clear();
    }
}
