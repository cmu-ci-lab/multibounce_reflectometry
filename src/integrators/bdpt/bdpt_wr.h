...
public:
    ...
    inline void putSample(const Point2 &sample, const Spectrum &spec, const Float &sampleDensity) {
        m_block->put(sample, spec, 1.0f, sampleDensity);
    }

    inline void putLightSample(const Point2 &sample, const Spectrum &spec) {
        m_lightImage->put(sample, spec, 1.0f, 0.0f);
    }
