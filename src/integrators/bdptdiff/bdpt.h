...
struct BDPTConfiguration {
    ...
    int rrDepth;
    int diffParameter;
    std::string diffParameterS;
    std::string diffRefName;
    bool sparseDifferentials;
    bool pathFiltering;
    int tFilter;
    int sFilter;
    bool angularHistogram;
    int ignoreIndex;
    ...
    inline BDPTConfiguration(Stream *stream) {
        ...
        diffParameter = stream->readInt();
        diffParameterS = stream->readString();
        diffRefName = stream->readString();
        pathFiltering = stream->readBool();
        angularHistogram = stream->readBool();
        sFilter = stream->readInt();
        tFilter = stream->readInt();
        ignoreIndex = stream->readInt();
    }
    ...
    inline void serialize(Stream *stream) const {
        ...
        stream->writeInt(diffParameter);
        stream->writeString(diffParameterS);
        stream->writeString(diffRefName);
        stream->writeBool(pathFiltering);
        stream->writeBool(angularHistogram);
        stream->writeInt(sFilter);
        stream->writeInt(tFilter);
        stream->writeInt(ignoreIndex);
    }

    void dump() const {
        SLog(EDebug, "Bidirectional differential path tracer configuration:");
        ...
        SLog(EDebug, "   Parameter to differentiate  : %i", diffParameter);
        SLog(EDebug, "   Path Filtering enabled : %i", (pathFiltering) ? 1 : 0);
        SLog(EDebug, "   Histogram dump enabled : %i", (angularHistogram) ? 1 : 0);
        if (pathFiltering)
            SLog(EDebug, "       S filter index : %i", (sFilter) ? 1 : 0);
            SLog(EDebug, "       T filter index : %i", (tFilter) ? 1 : 0);
        SLog(EDebug, "   Ignore Index                : %i", ignoreIndex);
