...
BDPTDiffWorkResult::BDPTDiffWorkResult(const BDPTConfiguration &conf,
        const ReconstructionFilter *rfilter, 
        const Sampler *sampler,
        Vector2i blockSize, bool isReductor, const float* reductor, Vector2i reductorSize) {
    ...
    Log(EInfo, "Created Work Result: %d x %d", blockSize.x, blockSize.y);
    //if (isReductor) {
    m_block = new ReductorImageBlock(Bitmap::ESpectrumAlphaWeight, blockSize, rfilter, sampler, -1, true, reductor, reductorSize, conf.ignoreIndex);
    /*} else {
        m_block = new SparseImageBlock(Bitmap::ESpectrumAlphaWeight, blockSize, rfilter, -1, true);
    }*/
    ...
    if (conf.lightImage) {
        ...
        //Log(EError, "Light images currently not supported. Please use <boolean name='lightImage' value='false'/> 
        //    in your XML for the integrator");
...
BDPTDiffWorkResult::~BDPTDiffWorkResult() { }

void BDPTDiffWorkResult::put(const BDPTDiffWorkResult *workResult) {
#if BDPT_DEBUG == 1
    for (size_t i=0; i<m_debugBlocks.size(); ++i)
...
void BDPTDiffWorkResult::dump(const BDPTConfiguration &conf,
...
void BDPTDiffWorkResult::load(Stream *stream) {
#if BDPT_DEBUG == 1
    for (size_t i=0; i<m_debugBlocks.size(); ++i)
...
void BDPTDiffWorkResult::save(Stream *stream) const {
#if BDPT_DEBUG == 1
    for (size_t i=0; i<m_debugBlocks.size(); ++i)
...
std::string BDPTDiffWorkResult::toString() const {
    return m_block->toString();
}

MTS_IMPLEMENT_CLASS(BDPTDiffWorkResult, false, WorkResult)
MTS_NAMESPACE_END
