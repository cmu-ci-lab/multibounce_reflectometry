 ...
 * }
 */
class BDPTDiffIntegrator : public Integrator {
public:
	BDPTDiffIntegrator(const Properties &props) : Integrator(props) {
        ...
        //m_config.diffParameter = props.getInteger("diffParameterI", -1);

        // String version, if necessary.
        //m_config.diffParameterS = props.getString("diffParameter", "");

        // BSDF Reference ID, if necessary.
        //m_config.diffRefName = props.getString("diffRefName", "");

        m_config.sparseDifferentials = props.getBoolean("sparseDifferentials", false);
        m_config.pathFiltering = props.getBoolean("pathFiltering", false);
        m_config.sFilter = props.getInteger("sFilter", 1);
        m_config.tFilter = props.getInteger("tFilter", 1);
        m_config.angularHistogram = props.getBoolean("angularHistogram", false);
        m_config.ignoreIndex = props.getInteger("ignoreIndex", -1);
	...
	BDPTDiffIntegrator(Stream *stream, InstanceManager *manager)
    ...
    bool preprocess(const Scene *scene, RenderQueue *queue,
        ...
        /*if(m_config.diffRefName != "") {

            auto refObjects = scene->getReferencedObjects();
            bool foundRef = false;
            for(int i = 0; i < refObjects.size(); i++) {

                if( refObjects[i]->getID() == m_config.diffRefName ) {
                    foundRef = true;
                    auto robj = refObjects[i];
                    auto slist = reinterpret_cast<BSDF*>(robj.get())->getDifferentiableParameterNames();
                    bool found = false;
                    for(int j = 0; j < slist.size(); j++) {
                        if(slist[j] == m_config.diffParameterS) {
                            // Found a parameter
                            m_config.diffParameter = j;
                            found = true;
                        }
                        Log(EInfo, "Found differential parameter %s:%d", slist[j].c_str(), j);
                    }

                    if(!found) {
                        Log(EError, "Couldn't find the differentiable parameter '%s' in BSDF '%s'", m_config.diffParameterS.c_str(), m_config.diffRefName.c_str());
                    }
                }
            ...
            if(!foundRef) {
                Log(EError, "Couldn't find the differentiable BSDF '%s'", m_config.diffRefName.c_str()); 
            }

        }
        
        if(m_config.diffParameter == -1)
            Log(EError, "Differential target parameter not specified / not found");

        Log(EInfo, "Differential Parameter selected: %s.%s:%d", m_config.diffRefName.c_str(), m_config.diffParameterS.c_str(), m_config.diffParameter);
    ...
    bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        // TODO: Check that the film supports sparse differentials.
        ...
        ref<BDPTDiffProcess> process = new BDPTDiffProcess(job, queue, m_config);
        m_process = process;
...
MTS_IMPLEMENT_CLASS_S(BDPTDiffIntegrator, false, Integrator)
MTS_EXPORT_PLUGIN(BDPTDiffIntegrator, "Bidirectional path tracer to compute differentials");
MTS_NAMESPACE_END
