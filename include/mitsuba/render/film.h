...
public:
    ...
    /// Returns if this film is a reductor film. Special case used for memory-optimized differential rendering
    virtual bool isReductor() { return false; }

    /// Returns the reductor texture associated with this film. Special case used for memory-optimized differential rendering
    virtual const float* getReductor() { return NULL; }

    /// Returns the reductor texture's size.
    virtual Vector2i getReductorSize() { return Vector2i(0,0); }
    ...
    /// Returns whether the film is sparse (a special film that is to be used only with sparse integrators).
