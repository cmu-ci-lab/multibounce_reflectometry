...
typedef std::map<int, Spectrum> DifferentialList;
/**
...
public:
    ...
    inline bool usesRayDifferentials() const {
		...
		/// Return whether this BSDF can be differentiated wrt any component
		inline bool isDifferentiable() const {
			return m_isDifferentiable;
		}
    ...
    virtual Spectrum eval(const BSDFSamplingRecord &bRec,
		...
		/**
		* \brief Evaluate the BSDF partial differential df(wi, wo, \theta)/(d\theta)
        *       wrt all differentiable parameters.
		*
		* This method allows to query the BSDF as a whole or pick out
		* individual components. When querying a smooth (i.e. non-degenerate)
		* component, it already multiplies the result by the cosine
		...
		*     ideally smooth dielectric coating).
		*/
		virtual std::map<int,Spectrum> eval_diff(const BSDFSamplingRecord &bRec,
    ...
    /**
    * \brief Return the number of parameters that can be differentiated
    *
    */
