...
public:
    ...
    ImageBlock(Bitmap::EPixelFormat fmt, const Vector2i &size,
            const ReconstructionFilter *filter = NULL, const Sampler *sampler = NULL, int channels = -1, bool warn = true);
     ...
     *    NaN or negative. A warning is also printed in this case
     */
    FINLINE bool put(const Point2 &pos, const Spectrum &spec, Float alpha, Float density) {
        Float temp[SPECTRUM_SAMPLES + 2];
        for (int i=0; i<SPECTRUM_SAMPLES; ++i)
            temp[i] = spec[i];
        temp[SPECTRUM_SAMPLES] = alpha;
        temp[SPECTRUM_SAMPLES + 1] = 1.0f;
        return put(pos, temp, density);
    }
     ...
     *    NaN or negative. A warning is also printed in this case
     */
    FINLINE bool put(const Point2 &_pos, const Float *value, Float density) {
...

        {
            ...
            for (int y=min.y, yr=0; y<=max.y; ++y, ++yr) {
                ...
                for (int x=min.x, xr=0; x<=max.x; ++x, ++xr) {
                    ...
                    if (density != 0.0f) {
                        Float targetDensity = m_sampler->getSampleCount(
                                Point2i(x + (m_offset.x - m_borderSize), y + (m_offset.y - m_borderSize))
                            );
                        importance = targetDensity / importance;
                    }

                    for (int k=0; k<channels; ++k)
                        *dest++ += weight * importance * value[k];
                }
            }
        }
    ...
    ref<ImageBlock> clone() const {
        ref<ImageBlock> clone = new ImageBlock(m_bitmap->getPixelFormat(),
            m_bitmap->getSize() - Vector2i(2*m_borderSize, 2*m_borderSize), m_filter, m_sampler, m_bitmap->getChannelCount());
        copyTo(clone);
        return clone;
    }
